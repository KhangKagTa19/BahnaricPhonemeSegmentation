...
filtered_df = df[df['number_phoneme'] == 4]
...
df.to_csv("4_phoneme_features.csv", index=False)

print("phoneme features saved to 4_phoneme_features.csv.")
...
with open('4_phoneme_features.csv', mode="r", encoding="utf-8", newline='') as csvfile:
    reader = csv.DictReader(csvfile)
    for row in reader:
        phoneme_1 = row['phoneme_1']
        phoneme_2 = row['phoneme_2']
        phoneme_3 = row['phoneme_3']
        phoneme_4 = row['phoneme_4']
        ...
        if phoneme_3 not in hierarchy_tree[phoneme_1][phoneme_2]:
            hierarchy_tree[phoneme_1][phoneme_2][phoneme_3] = []

        # Add phoneme 4 to the hierarchy tree if not already present
        if phoneme_4 not in hierarchy_tree[phoneme_1][phoneme_2][phoneme_3]:
            hierarchy_tree[phoneme_1][phoneme_2][phoneme_3].append(phoneme_4)
